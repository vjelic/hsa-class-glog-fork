...
INC_DIR=$LLVM_DIR/lib/clang/10.0.0/include
BITCODE_OPTS="\
...
for GFXIP in $TGT_LIST ; do
  ...
  $CLANG -cl-std=CL$OCL_VER -include $INC_DIR/opencl-c.h $BITCODE_OPTS -target amdgcn-amd-amdhsa -mcpu=$GFXIP -mno-code-object-v3 $TEST_NAME.cl -o $DST_DIR/$OBJ_FILE
